...
const { load, cutForSearch } = require('@node-rs/jieba');
...
search.init = async function (params) {
	...
	pubsub.on('nodebb-plugin-dbsearch-rsjieba:settings:save', (data) => {
...
search.reindex = async function () {
	await db.setObject('nodebb-plugin-dbsearch-rsjieba', {
		topicsIndexed: 0,
		postsIndexed: 0,
		messagesIndexed: 0,
		working: 1,
...
function tokenize(text) {
	// console.log(`nodebb-plugin-dbsearch-rsjieba=>text: ${text}`);
	const pattern = /[\u4e00-\u9fa5]/;
	if (pattern.test(text) === false) { // 无中文直接返回
		return text.toLowerCase();
	}
	const result = cutForSearch(text.toLowerCase());
	const chars = text.split('');
	let keys = Array.from(new Set(result.concat(chars)));
	keys = keys.filter(item => excludeChars.includes(item) !== true);
...
async function topicsSave(topics) {
	...
	await db.incrObjectFieldBy('nodebb-plugin-dbsearch-rsjieba', 'topicsIndexed', result.tids.length);
}
...
async function postsSave(posts) {
	...
	await db.incrObjectFieldBy('nodebb-plugin-dbsearch-rsjieba', 'postsIndexed', result.pids.length);
}
...
async function messagesSave(msgs) {
	...
	await db.incrObjectFieldBy('nodebb-plugin-dbsearch-rsjieba', 'messagesIndexed', result.mids.length);
}

async function searchRemove(key, ids) {
	await db.searchRemove(key, ids);
	if (key === 'topic') {
		await db.incrObjectFieldBy('nodebb-plugin-dbsearch-rsjieba', 'topicsIndexed', -ids.length);
	} else if (key === 'post') {
		await db.incrObjectFieldBy('nodebb-plugin-dbsearch-rsjieba', 'postsIndexed', -ids.length);
	} else if (key === 'chat') {
		await db.incrObjectFieldBy('nodebb-plugin-dbsearch-rsjieba', 'messagesIndexed', -ids.length);
	}
}
...
async function reIndexPids(pids, topic) {
	if (!Array.isArray(pids) || !pids.length) {
		winston.warn('[nodebb-plugin-dbsearch-rsjieba] invalid-pid, skipping');
		return;
	}
...
async function save(req, res) {
	if (utils.isNumber(req.body.postLimit) && utils.isNumber(req.body.topicLimit)) {
		...
		await db.setObject('nodebb-plugin-dbsearch-rsjieba', data);
		...
		pubsub.publish('nodebb-plugin-dbsearch-rsjieba:settings:save', pluginConfig);
...
async function getPluginData() {
	const data = await db.getObject('nodebb-plugin-dbsearch-rsjieba') || {};
...
socketAdmin.plugins.dbsearch.changeLanguage = async function (socket, language) {
	...
	await db.setObject('nodebb-plugin-dbsearch-rsjieba', { indexLanguage: language });
};
